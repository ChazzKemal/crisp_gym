...
from crisp_gym.deploy.session import (
    build_env,
    phase_gil_hygiene,
    phase_home,
    phase_pin_gripper_speed,
    phase_publish_channels,
    phase_scaler,
    phase_start_sender,
...
def main() -> int:
    ...
    phase_home(env, args)
    phase_switch_controller(env, args)
    scaler = phase_scaler(env, args)
    phase_pin_gripper_speed(env, args)
    phase_gil_hygiene(env, args)
    ch = phase_publish_channels(env, args)
    ...
    py_pose_pub = ch.py_pose_pub
    sender, q = phase_start_sender(env, args, scaler, ch)
    run_started_at, out_dir, video_recorders = phase_video_and_delay(
