#!/usr/bin/env python3
"""UR10e crisp_gym environment — mocap-driven data recording.
...
def silence_env_target_publishers(env) -> None:
    ...
    ``crisp_py.Robot`` unconditionally creates a publisher on
    ``target_pose_topic`` / ``target_joint_topic`` plus a timer that ticks at
    ``publish_frequency`` and re-sends the robot's internal target. When an
    external teleop node is also publishing to ``/target_pose`` this causes
    a dual-publisher conflict that the cartesian_controller resolves by
    averaging, so the arm barely follows mocap.

    We replace ``publish()`` on the instance's publishers with a no-op. The
    publishers are still advertised in the DDS graph (they were created
    during ``Robot.__init__`` and can't be cleanly destroyed from outside
    crisp_py), but they emit nothing — so from the controller's perspective
    only the mocap tracker is sending targets.
    """
